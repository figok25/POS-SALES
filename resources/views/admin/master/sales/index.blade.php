<x-admin-layout>
    <div class="p-6">
        ...
        <form method="GET" class="mb-4">
            <input type="text" name="q" value="{{ $search }}" placeholder="Cari Sales..." class="border rounded px-3 py-2 text-sm w-64">
                    ...
                    @forelse ($items as $item)
                        <tr class="border-b">
                            ...
                            <td class="px-3 py-2 text-right space-x-2">
                                <a href="{{ route('admin.master.sales.edit', $item) }}" class="text-blue-600 hover:underline">Edit</a>
                                <form action="{{ route('admin.master.sales.destroy', $item) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data ini?')">
                                    @csrf
                                    @method('DELETE')
