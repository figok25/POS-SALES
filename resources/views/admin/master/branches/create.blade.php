<x-admin-layout>
    <div class="p-6 max-w-xl">
        <h1 class="text-xl font-semibold mb-4">Tambah Branch</h1>
        <form method="POST" action="{{ route('admin.master.branches.store') }}" class="bg-white p-4 rounded shadow">
            @csrf
            @php($item = null)
            
        <div class="mb-4">
            <label class="block text-sm font-medium mb-1">Company *</label>
            <select name="company_id" class="w-full border rounded px-3 py-2 text-sm">
                <option value="">-- Pilih Company --</option>
                @foreach ($companys as $opt)
                    <option value="{{ $opt->id }}" @selected(old('company_id', $item->company_id ?? null) == $opt->id)>{{ $opt->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <div class="flex justify-end gap-2">
                <a href="{{ route('admin.master.branches.index') }}" class="px-3 py-2 text-sm rounded border">Batal</a>
